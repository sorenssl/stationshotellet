...
- [x] **Checklist SEO round 2 (2026-07-16)** — rollback tag `pre-seo2-2026-07-16`. Script: `upgrade_20260716_checklist.py` (one-shot). Driven by global `knowledge/website/CHECKLIST.md`. Added: `llms.txt` (AI-search/GEO), `terms.html` (EN+SV booking terms + OFFICIAL CHANNELS anti-scam notice, linked from all footers), `favicon.svg`, referrer meta, titles trimmed ≤60 chars, money keywords in H2s, visible freshness stamp. **Host limits (GitHub Pages, conscious):** HSTS/X-Content-Type-Options headers impossible — fix = put Cloudflare proxy in front (DNS at Loopia); DMARC TXT record missing — Soren adds at Loopia: `_dmarc.stationshotellet.com TXT "v=DMARC1; p=reject;"` (domain sends no mail — pure anti-spoofing). EUR offers in schema = conscious NO (rate floats; SEK is billed currency).
